...
#        print"prepare_report_contents acc_lines: \n\n\n",acc_lines
        ...
        self.sheet.write_string(self.row_pos, 3, _('Customer'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 4, _('QtyIn'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 5, _('QtyOut'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 6, _('Price'),
                                self.format_header)
        self.sheet.write_string(self.row_pos, 7, _('Discount'),
                                self.format_header)
            ...
            for sub_line in acc_lines:
                ...
                if sub_line.get('ref_no')!='Total':
                    ...
                    self.sheet.write_number(self.row_pos, 4,
                                            float(sub_line.get('qty_in')),self.line_header_light)
                    self.sheet.write_number(self.row_pos, 5,
                                            float(sub_line.get('qty_out')),self.line_header_light)
                ...
                if sub_line.get('ref_no')=='Total':
                    ...
                    self.sheet.write_string(self.row_pos, 2, '',
                                            self.format_header_right)
                    self.sheet.write_string(self.row_pos, 3, sub_line.get('name'),
                                            self.format_header_right)
                    self.sheet.write_number(self.row_pos, 4,
