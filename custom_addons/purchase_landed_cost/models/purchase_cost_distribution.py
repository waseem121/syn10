...
#        self.amount_total = round(self.total_purchase + self.total_expense, 2)
        self.amount_total = round(self.total_purchase + self.total_expense, 10)
...
#        self.calculated_amount_total = round(calculated_amount_total, 2)
        self.calculated_amount_total = round(calculated_amount_total, 10)
...
#         self.total_purchase = sum([(x.standard_price_new * x.product_qty) for x in self.cost_lines])
    ...
    total_purchase = fields.Float(
        ...
        digits=(12, 10), string='Total purchase')
    ...
    calculated_amount_total = fields.Float(
        ...
        digits=(12, 10), string='Calculated Total')
        
    difference_amount = fields.Float(
        compute=_compute_difference_amount_total,
        digits=(12, 10), string='Difference')
        
    total_expense = fields.Float(
        compute=_compute_total_expense,
        digits=(12, 10), string='Total expenses')
...
#                print"disss action_done standard_price_new: ",line.standard_price_new
...
#        self.total_amount = self.standard_price_old * self.product_qty
        if self.purchase_id.currency_id.id != self.distribution.currency_id.id:
            ...
            if exchange_rate:
                local_subtotal = self.purchase_line_id.price_subtotal / exchange_rate
            else:
                local_subtotal = self.purchase_line_id.price_subtotal / 1
        else:
            local_subtotal = self.purchase_line_id.price_subtotal / 1
...
#            qty_default_uom = self.purchase_line_id.product_uom._compute_quantity(self.purchase_line_id.product_qty, self.purchase_line_id.product_id.uom_id)
        ...
        if qty_default_uom > 0:
            self.standard_price_old = local_subtotal / qty_default_uom
        else:
            self.standard_price_old = local_subtotal
                
            
#        if qty_default_uom > 0:
#            print"inside iff"
#            print"self.purchase_line_id.price_subtotal: ",self.purchase_line_id.price_subtotal
#            print"qty_default_uom: ",qty_default_uom
...
#        print"self.distribution.currency_id: ",self.distribution.currency_id
#        if self.purchase_id.currency_id.id != self.distribution.currency_id.id:
...
#                self.standard_price_old = self.purchase_id.currency_id.compute(self.standard_price_old, self.distribution.currency_id, round=False)
    ...
    product_price_unit = fields.Float(
        string='Unit price', related='move_id.price_unit', digits=(12, 12))
    expense_lines = fields.One2many(
...
class PurchaseCostDistributionExpense(models.Model):
    ...
    expense_amount_currency = fields.Float(
        string='Expense amount (Base Currency)', digits=(12, 10),
