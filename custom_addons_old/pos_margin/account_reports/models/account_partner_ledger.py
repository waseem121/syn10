...
class ReportPartnerLedger(models.AbstractModel):
    ...
    def group_by_partner_id(self, line_id):
        ...
        initial_bal_date_to = datetime.strptime(self.env.context['date_from_aml'],
                                                DEFAULT_SERVER_DATE_FORMAT) + timedelta(days=-1)
        initial_bal_results = self.with_context(
            date_to=initial_bal_date_to.strftime(DEFAULT_SERVER_DATE_FORMAT)).do_query(line_id)
    ...
    def _lines(self, line_id=None):
        ...
        grouped_partners = self.with_context(date_from_aml=context['date_from'], date_from=context['date_from'] and
                                                                                           company_id.compute_fiscalyear_dates(
                                                                                               datetime.strptime(
                                                                                                   context['date_from'],
                                                                                                   DEFAULT_SERVER_DATE_FORMAT))[
                                                                                               'date_from'] or None).group_by_partner_id(
            line_id)  # Aml go back to the beginning of the user chosen range but the amount on the partner line should go back to either the beginning of the fy or the beginning of times depending on the partner
        ...
        for partner in sorted_partners:
            ...
            if partner in context['context_id']['unfolded_partners'] or unfold_all:
                ...
                domain_lines[:0] = [{
                    ...
                    'columns': ['', '', '', '', self._format(initial_debit), self._format(initial_credit),
