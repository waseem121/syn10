...
class StockReport(models.TransientModel):
    ...
    price_selection = fields.Selection([('show_price', 'Show Price'), ('show_cost', 'Show Cost'), ('both', 'Show Price and Cost')], 
    ...
    def generate_stock_report_xlsx(self):
        ...
        if self.date:
            ...
            worksheet.write_merge(9, 9, 2, 3,'Date : ' + str(datetime.strftime(datetime.today(), '%d-%m-%Y')), style=stylePC)
                ...
                for k,v in value.items():
                    ...
                    for each_value in v:
                        ...
                        if self.date and self.date_selection == 'on_date':
                            ...
                            cost_sum = cost_res[0].get('cost') if cost_res and cost_res[0] and cost_res[0].get('cost') else 0.0
                        else:
                    ...
                    if each_value['product'] == k:
                        ...
                        if self.price_selection == 'both':
                            ...
                            worksheet.write_merge(categ_row, categ_row, 6+i, 6+i, '%.3f' %product_dict['cost_total'])
                            worksheet.write_merge(categ_row, categ_row, 7+i, 7+i, '%.3f' %product_dict['value'])
                            worksheet.write_merge(categ_row, categ_row, 8+i, 8+i, '%.3f' %product_dict['value_total'])
                        elif self.price_selection == 'show_price':
                            worksheet.write_merge(categ_row, categ_row, 5+i, 5+i, '%.3f' %product_dict['value'])
                            worksheet.write_merge(categ_row, categ_row, 6+i, 6+i, '%.3f' %product_dict['value_total'])
                        elif self.price_selection == 'show_cost':
                            worksheet.write_merge(categ_row, categ_row, 5+i, 5+i, '%.3f' %product_dict['cost'])
                            worksheet.write_merge(categ_row, categ_row, 6+i, 6+i, '%.3f' %product_dict['cost_total'])
