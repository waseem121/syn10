...
class StockReportLot(models.TransientModel):
    ...
    def generate_stock_report_lot_xlsx(self):
        ...
        date_on = datetime.strptime(self.based_on_date, "%Y-%m-%d").strftime('%d-%m-%Y') if self.based_on == 'on_date' else datetime.strftime(datetime.today(), '%d-%m-%Y')
    ...
    def _query_detail(self):
        ...
        if self.exp_date_from and self.exp_date_to:
            ...
            query1 = """spl.life_date >= '%s' AND
                        spl.life_date <= '%s' AND""" % (ex_date_from.replace(hour=00, minute=00, second=00), ex_date_to.replace(hour=23, minute=59, second=59))
#         if self.based_on:
...
#                 query2 = """sm.date <= '%s' AND""" % (b_date)
        ...
        for key_main,value in final_dict.items():
            ...
            for prod,prodval in value.items():
                ...
                grouper = itemgetter('product', 'batch_no', 'ex_date', 'id', 'description', 'barcode', 'brand', 'item_code', 'product_category')
                result = []
                for key, grp in groupby(sorted(prodval, key = grouper), grouper):
                    temp_dict = dict(zip(['product', 'batch_no', 'ex_date', 'id', 'description', 'barcode', 'brand', 'item_code', 'product_category'], key))
                    temp_dict["cost"] = temp_dict["sale_price"] = temp_dict["value"] = temp_dict["net_value"] = 0.0
                    temp_dict["qty"] = 0
                    for each in grp:
                        if each.get('product_id',False):
                            product_price_history_id = self.env['product.price.history'].search([('product_id','=',each.get('product_id'))],limit=1)
                            each['cost'] = product_price_history_id.cost if product_price_history_id and product_price_history_id.cost else 0.0
                            each["value"] = each['qty'] * each['cost']
                        temp_dict["qty"] += each["qty"]
                        if not temp_dict["cost"]:
                            temp_dict["cost"] += each["cost"] or 0.0
                        else:
                            temp_dict["cost"] = (temp_dict["cost"] + each["cost"] or 0.0) / 2 if temp_dict["cost"] != each["cost"] or 0.0 else temp_dict["cost"]
                        if not temp_dict["sale_price"]:
                            temp_dict["sale_price"] += each["sale_price"]
                        else:
                            temp_dict["sale_price"] = (temp_dict["sale_price"] + each["sale_price"]) / 2 if temp_dict["sale_price"] != each["sale_price"] else temp_dict["sale_price"]
