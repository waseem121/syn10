...
class Stock_Rotation_Report(models.TransientModel):
    ...
    def print_stock_rotation_report(self):
        ...
        self.get_stock_rotation_report(from_date,to_date,warehouses,all_products)
        if self.datas:
            return {
    ...
    @api.multi
    def add_headings(self,from_date,to_date,warehouses,workbook,header_bold,body_style,qty_cell_style,value_style,blank_cell_style):
        """
    ...
    @api.multi
    def get_all_locations(self,warehouse):
        """
    ...
    def find_sale_qty_in_duration(self,from_date,to_date,warehouse,location,product_id):
        """
            ...
            in given duration. for specific location of warehouse.
        """
        query="""select sum(product_uom_qty) from stock_move mv 
                Inner join stock_location sl on sl.id = mv.location_dest_id and sl.usage='customer'
                where state='done' and mv.location_id in (%s) and product_id = %s and warehouse_id= %s and date between '%s 00:00:00' and '%s 23:59:59'
                """%(','.join(str(x) for x in location),product_id.id,warehouse.id,from_date,to_date)
        self._cr.execute(query)
        result = self._cr.fetchall()
        return result
    ...
    @api.multi
    def prepare_data(self,from_date,to_date,warehouses,all_products):
        """
...
#                 here from result of inventory at date we are seaching for specific product.
    ...
    def print_all_rotation_sheet_data(self,product_data_dict,workbook,worksheet_all_stock_rotation,body_style,qty_cell_style,value_style,blank_cell_style,value_style2):
        ...
        return True
    @api.multi
    def get_stock_rotation_report(self,from_date,to_date,warehouses,all_products):
        """
        ...
        workbook,sheet_data,row_data = self.add_headings(from_date,to_date,warehouses,workbook,header_bold,body_style,qty_cell_style,value_style,blank_cell_style)
        data_dict = self.prepare_data(from_date, to_date, warehouses, all_products)
